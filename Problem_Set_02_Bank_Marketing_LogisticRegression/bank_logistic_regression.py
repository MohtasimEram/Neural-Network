...
====================================================================
This script analyses the Bank Marketing dataset and builds a Logistic
Regression model to predict whether a customer will subscribe to a
term deposit ('yes') or not ('no').
...
# 4a. Age distribution by subscription
...
# 4d. Balance vs subscription (box plot)
...
# 4e. Contact duration distribution
fig, ax = plt.subplots(figsize=(9, 5))
...
# 5a. Encode target variable
df_processed["y"] = df_processed["y"].map({"no": 0, "yes": 1})

# 5b. Encode categorical features using Label Encoding
label_encoders = {}
for col in cat_cols:
    if col == "y":
        continue
...
print("\n[INFO] Features scaled using StandardScaler (zero mean, unit variance)")
...
# ─── 16. FEATURE IMPORTANCE (Coefficients) ─────────────────────────────────────
...
# ─── 18. MARITAL STATUS vs SUBSCRIPTION ─────────────────────────────────────────
